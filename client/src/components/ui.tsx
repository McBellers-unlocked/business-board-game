...
export function Button({ className, variant = "primary", ...rest }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: "primary" | "secondary" | "ghost" | "danger" }) {
  const cls = clsx(
    ...
    variant === "secondary" && "bg-white text-ink-900 border border-ink-300 hover:bg-ink-100",
...
export function Input(props: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={clsx(
        "block w-full rounded-md border border-ink-300 bg-white px-3 py-2 text-sm shadow-sm",
